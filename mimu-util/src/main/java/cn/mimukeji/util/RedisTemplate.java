...
public final class RedisTemplate {
    ...
    public void init(){
        ...
        jedisPool = new JedisPool(config, this.ip, this.port, this.timeout, this.password, this.database);
    };
    ...
    public RedisTemplate(String ip, String password, int maxActive, int maxIdle, int maxWait, int timeout, int port,
            int database) {
     ...
     *            key前缀
     */
    public void batchDel(String preKey) {
        Jedis jedis = null;
        try {
            if (jedisPool != null) {
                jedis = jedisPool.getResource();
